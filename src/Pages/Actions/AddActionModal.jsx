...
export default function AddActionModal({ open, onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await onCreated({ title, description, assigned_to: assignedTo, due_date: dueDate || null, status });
      reset();
      onClose();
    } catch (err) {
    ...
    <Dialog open={open} onClose={handleClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-md bg-white rounded-xl p-6">
          <DialogTitle className="text-lg font-bold text-gray-900">Add Action</DialogTitle>
